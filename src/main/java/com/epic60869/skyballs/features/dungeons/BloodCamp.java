...
public final class BloodCamp {
    ...
    public static Component watcherBarName(Component name, float progress) {
        ...
        if (!"The Watcher".equals(SkyBallsLocation.strip(name.getString()))) return name;
    ...
    private static void onChat(String text) {
        ...
        if (BLOOD_START.matcher(text).matches()) {
            ...
            if (config.moveMessage) SkyBallsAlerts.chat(Component.literal(msg).withStyle(ChatFormatting.RED));
    ...
    private static void onServerTick() {
        ...
        if (killTitleIn > 0 && --killTitleIn == 0) {
            FeatureConfigs.BloodCamp config = config();
            if (config != null && config.killTitle) SkyBallsAlerts.title(Component.literal("Kill Mobs").withStyle(ChatFormatting.RED, ChatFormatting.BOLD), Component.empty());
            moveSeconds = -1;
        }
        FeatureConfigs.BloodCamp config = config();
        if (config == null || !config.killTimers || !inClear()) return;
