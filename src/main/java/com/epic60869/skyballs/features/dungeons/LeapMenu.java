...
public final class LeapMenu {
    ...
    public static void init() {
        ...
        SkyBallsChat.onChat(message -> {
            ...
            if (config == null || !config.announce || !SkyBallsLocation.inDungeon()) return;
    ...
    public static boolean isActive(AbstractContainerScreen<?> screen) {
        FeatureConfigs.LeapMenu config = config();
        if (config == null || !config.enabled || !SkyBallsLocation.inDungeon()) return false;
    ...
    private static void leapTo(AbstractContainerScreen<?> screen, int quadrant) {
        ...
        if (t == null) return;
        if (!t.alive()) {
            SkyBallsAlerts.chat(Component.literal("This player is dead, you can not leap to them.").withStyle(ChatFormatting.RED));
            return;
        }
        Minecraft mc = Minecraft.getInstance();
        ...
        for (Slot slot : screen.getMenu().slots) {
            ...
            SkyBallsAlerts.chat(Component.literal("Teleporting to " + t.name() + ".").withStyle(ChatFormatting.GRAY));
            return;
        }
    }
}
