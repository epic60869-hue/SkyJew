				...
				if (!itemUuid.isEmpty()) {
					...
					if (material == null && pattern == null) {
						if (customArmorTrims.containsKey(itemUuid)) {
							...
							source.sendFeedback(Compat.PREFIX.get().append(Component.translatable("skyballs.customArmorTrims.removed")));
						} else {
							source.sendFeedback(Compat.PREFIX.get().append(Component.translatable("skyballs.customArmorTrims.neverHad")));
						}
					} else {
						// Ensure that the material & trim are valid
						ArmorTrimId trimId = new ArmorTrimId(material, pattern);
						if (TRIMS_CACHE.get(trimId) == null) {
							source.sendError(Compat.PREFIX.get().append(Component.translatable("skyballs.customArmorTrims.invalidMaterialOrPattern")));

							return Command.SINGLE_SUCCESS;
						}

						CustomConfigManager.update(config -> config.general.customArmorTrims.put(itemUuid, trimId));
						source.sendFeedback(Compat.PREFIX.get().append(Component.translatable("skyballs.customArmorTrims.added")));
					}
				} else {
					source.sendError(Compat.PREFIX.get().append(Component.translatable("skyballs.customArmorTrims.noItemUuid")));
				}
			} else {
				source.sendError(Compat.PREFIX.get().append(Component.translatable("skyballs.customArmorTrims.notAnArmorPiece")));
				return Command.SINGLE_SUCCESS;
			}
		} else {
			source.sendError(Compat.PREFIX.get().append(Component.translatable("skyballs.customArmorTrims.unableToSetTrim")));
		}
