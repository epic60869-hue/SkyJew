...
import com.epic60869.skyballs.custom.screen.name.CustomizeNameWidget;
...
public class ItemTab extends GridLayoutTab {
	...
	public ItemTab(CustomizeScreen parentScreen) {
		super(Component.translatable("skyballs.customization.item"));
		layout.spacing(5);
		...
		modelField.setHint(Component.translatable("skyballs.customization.item.modelOverride").withStyle(ChatFormatting.ITALIC).withStyle(ChatFormatting.GRAY));
		...
		linearLayout.addChild(Button.builder(Component.translatable("skyballs.customization.item.selectModel"), _ -> {
		...
		layout.addChild(new StringWidget(Component.translatable("skyballs.customization.nothingCustomizable"), Minecraft.getInstance().font),
	...
	private Component getGlintText() {
		return Component.translatable("skyballs.customization.item.glint", switch (glintState) {
			case DEFAULT -> Component.translatable("skyballs.customization.item.glint.default");
