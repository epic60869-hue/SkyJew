...
import com.epic60869.skyballs.custom.CustomConfigManager.GeneralConfig;
...
public class CustomizeScreen extends Screen {
	...
	public static void initThings() {
		ClientCommandRegistrationCallback.EVENT.register((dispatcher, _) -> {
				for (String root : Compat.COMMAND_ROOTS) dispatcher.register(
		...
		ScreenEvents.AFTER_INIT.register((_, screen, _, _) -> {
			if (Compat.isOnSkyblock() && com.epic60869.skyballs.SkyBallsConfig.current() != null && com.epic60869.skyballs.SkyBallsConfig.current().general.itemCustom.showCustomizeButton && screen instanceof InventoryScreen inventoryScreen) {
				// Skyblocker repositions on a recipe book toggle callback; SkyBalls re-anchors the button every frame instead.
	...
	public CustomizeScreen(@Nullable Screen previousScreen, boolean item) {
		super((Math.random() < 0.01 ? Component.translatable("skyballs.customization.titleSecret") : Component.translatable("skyballs.customization.title")).withStyle(ChatFormatting.GRAY).withStyle(style -> style.withShadowColor(0)));
	...
	public void backupConfigs(ItemStack stack) {
		...
		GeneralConfig general = CustomConfigManager.get().general;
	...
	public void onClose() {
		...
		CustomConfigManager.update(Consumers.nop());
	}
	...
	private static class CustomizeButton extends AbstractWidget {
		// thanks to @yuflow
		private static final Identifier TEXTURE = Compat.id("armor_customization_screen/button");

		private final InventoryScreen inventoryScreen;

		private CustomizeButton(InventoryScreen inventoryScreen) {
		...
		private void updatePosition() {
			SkyBallsContainerScreenAccessor accessor = (SkyBallsContainerScreenAccessor) inventoryScreen;
			setPosition(accessor.skyballs$getLeftPos() + 63, accessor.skyballs$getTopPos() + 10);
		}
