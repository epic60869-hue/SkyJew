...
import com.epic60869.skyballs.custom.screen.name.visitor.GetClickedPositionVisitor;
import com.epic60869.skyballs.custom.screen.name.visitor.GetRenderWidthVisitor;
import com.epic60869.skyballs.custom.screen.name.visitor.GetStyleVisitor;
import com.epic60869.skyballs.custom.screen.name.visitor.InsertTextVisitor;
import com.epic60869.skyballs.custom.screen.name.visitor.SetStyleVisitor;
...
public class CustomizeNameWidget extends AbstractContainerWidget {
	...
	public CustomizeNameWidget(Screen parent) {
		...
		builder.add(grid.addChild(Button.builder(Component.translatable("skyballs.customItemNames.screen.customColor"), _ ->
				client.gui.setScreen(ColorPopup.create(parent, color -> setStyle(Style.EMPTY.withColor(color))))
		).size(48, 16).build(), 2, 17, 1, 3));
		builder.add(grid.addChild(Button.builder(Component.translatable("skyballs.customItemNames.screen.gradientColor"), _ ->
				client.gui.setScreen(ColorPopup.createGradient(parent, this::createGradient))
		).size(48, 16).build(), 3, 17, 1, 3));
		builder.add(grid.addChild(new StringWidget(20 * 16, textRenderer.lineHeight, Component.translatable("skyballs.customItemNames.screen.howToRemove").withStyle(ChatFormatting.ITALIC, ChatFormatting.GRAY), textRenderer)/*.alignLeft()*/, 4, 0, 1, 20, LayoutSettings.defaults().paddingTop(2)));
