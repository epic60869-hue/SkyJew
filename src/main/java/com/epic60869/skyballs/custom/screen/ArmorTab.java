...
import static com.epic60869.skyballs.custom.screen.CustomizeScreen.CLIENT;
...
public class ArmorTab extends GridLayoutTab implements Closeable {
	...
	public ArmorTab(CustomizeScreen parent) {
		super(Component.translatable("skyballs.customization.armor"));
		this.parent = parent;
		layout.rowSpacing(PADDING / 2).columnSpacing(PADDING);

		List<ItemStack> list = Compat.getArmor(CLIENT.player);
		...
		if (nothingCustomizable) {
			layout.addChild(new StringWidget(Component.translatable("skyballs.customization.nothingCustomizable"), textRenderer), 0, 1, 2, 1, p -> p.alignVerticallyMiddle().alignHorizontallyCenter());
		}
	...
	private static boolean canEdit(ItemStack stack) {
		boolean hasUuid = !Compat.uuid(stack).isEmpty();
	...
	private void updateWidgets() {
		...
		if (CustomConfigManager.get().general.customArmorModel.containsKey(uuid)) {
	...
	private class PieceSelectionWidget extends AbstractWidget {
		private static final Identifier HOTBAR_TEXTURE = Compat.id("armor_customization_screen/mini_hotbar");
		private static final Identifier HOTBAR_SELECTION_TEXTURE = Compat.id("hotbar_selection_full");
	...
	private class ModelFieldContainer extends AbstractContainerWidget {
		private final Component text = Component.translatable("skyballs.customization.armor.modelOverride").withStyle(ChatFormatting.ITALIC).withStyle(ChatFormatting.GRAY);
		...
		private ModelFieldContainer(int width, int height) {
			...
			field = containerLayout.addChild(new IdentifierTextField(width - 10, 20, identifier -> {
				String uuid = Compat.uuid(armor[selectedSlot]);
			...
			field.setTooltip(Tooltip.create(Component.translatable("skyballs.customization.armor.modelOverride.tooltip")));
