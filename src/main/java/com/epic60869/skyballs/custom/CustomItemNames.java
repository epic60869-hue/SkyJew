...
public class CustomItemNames {
	...
	private static int openScreen(FabricClientCommandSource source) {
		if (!Compat.isOnSkyblock()) {
			source.sendError(Compat.PREFIX.get().append(Component.translatable("skyballs.customItemNames.notOnSkyblock")));
			return 0;
		}
		ItemStack handStack = source.getPlayer().getMainHandItem();
		if (handStack.isEmpty()) {
			source.sendError(Compat.PREFIX.get().append(Component.translatable("skyballs.customItemNames.noItem")));
			return 0;
		}
		if (Compat.uuid(handStack).isEmpty()) {
			source.sendError(Compat.PREFIX.get().append(Component.translatable("skyballs.customItemNames.noItemUuid")));
			return 0;
		}
	...
	private static int renameItem(FabricClientCommandSource source, Component text) {
		if (Compat.isOnSkyblock()) {
			...
			if (!itemUuid.isEmpty()) {
				...
				source.sendFeedback(Compat.PREFIX.get().append(Component.translatable("skyballs.customItemNames.added")));
