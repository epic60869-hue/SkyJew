...
public class CustomAnimatedHelmetTextures {
	...
	public static void init() {
		RepoItems.runAsync(CustomAnimatedHelmetTextures::loadAnimatedHeads);
		ClientTickEvents.END_CLIENT_TICK.register(_ -> ticks++);
	}

	private static void loadAnimatedHeads() {
		try {
			String data = RepoItems.neuRepoFile("constants/animatedskulls.json");
	...
	public static String formatName(String id) {
		return java.util.Arrays.stream(id.replace('_', ' ').toLowerCase(java.util.Locale.ROOT).split(" "))
	...
	private record AnimatedHead(int tickThreshold, List<ResolvableProfile> frames) {
		...
		private static ResolvableProfile fromString(String string) {
			...
			PropertyMap propertyMap = Compat.propertyMapWithTexture(split[1]);
	...
	private static class AnimatedHeadStateTracker {
		...
		private void advanceTo(int ticks) {
			// Start loading every frame's skin at once (SkyBalls). Frames that have not loaded yet
