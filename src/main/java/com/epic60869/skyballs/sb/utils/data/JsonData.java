package com.epic60869.skyballs.sb.utils.data;
		...
		if (createBackup) {
			...
			try {
				...
				if (extensionIndex >= 0) {
					...
					newFileName = fileName.substring(0, extensionIndex) + '_' + java.time.format.DateTimeFormatter.ofPattern("yyyy-MM-dd_HH-mm-ss").format(LocalDateTime.now()) + extension;
				} else {
					LOGGER.warn("[Skyblocker Json Data] No extension? {}", file);
					newFileName = fileName + '_' + java.time.format.DateTimeFormatter.ofPattern("yyyy-MM-dd_HH-mm-ss").format(LocalDateTime.now());
				}
