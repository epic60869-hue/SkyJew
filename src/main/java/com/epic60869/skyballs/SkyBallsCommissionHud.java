...
public final class SkyBallsCommissionHud {
    private static final Identifier ID = Identifier.fromNamespaceAndPath("skyballs", "commissions");
    ...
    private SkyBallsCommissionHud() {}

    public static void register(SkyBallsConfig cfg) {
        config = cfg;
        HudElementRegistry.addLast(ID, SkyBallsCommissionHud::extract);
    }
    ...
    private static void extract(GuiGraphicsExtractor g, net.minecraft.client.DeltaTracker delta) {
        ...
        if (config.mining.features.commissionsAreaOnly
            && !com.epic60869.skyballs.features.core.SkyBallsLocation.inMiningIsland()) return;
        updateFromTab();
        if (!commissions.isEmpty()) render(g, commissions, com.epic60869.skyballs.features.core.SkyBallsHuds.mapX(x(), width()), com.epic60869.skyballs.features.core.SkyBallsHuds.mapY(y(), height()));
    }
    ...
    private static void updateFromTab() {
        ...
        for (PlayerInfo entry : entries) {
            Component display = com.epic60869.skyballs.custom.util.Compat.rawTabName(entry);
