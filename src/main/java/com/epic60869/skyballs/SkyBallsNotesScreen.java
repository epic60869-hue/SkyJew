 ...
 */
public final class SkyBallsNotesScreen extends Screen {
    ...
    public SkyBallsNotesScreen(Path configDir) {
    ...
    protected void init() {
        if (text == null) {
            text = SkyBallsNotes.load(configDir).replace("\r\n", "\n").replace('\r', '\n');
        } else if (editor != null) {
    ...
    private void save() {
        dirty = false;
        SkyBallsNotes.save(configDir, text == null ? "" : text);
    }
