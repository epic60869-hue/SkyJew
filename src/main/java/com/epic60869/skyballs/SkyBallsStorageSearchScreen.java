 ...
 * inventory-style item grid. SkyBalls keeps its own Java implementation so
...
public final class SkyBallsStorageSearchScreen extends Screen {
    ...
    private List<SkyBallsStorageSearch.Result> results = List.of();
    ...
    private void refresh() {
        ...
        results = SkyBallsStorageSearch.search(
    ...
    public void extractRenderState(GuiGraphicsExtractor g, int mouseX, int mouseY, float delta) {
        ...
        for (int visible = 0; visible < rows() * columns(); visible++) {
            ...
            SkyBallsStorageSearch.Result result = results.get(index);
    ...
    public boolean mouseClicked(MouseButtonEvent event, boolean doubleClick) {
        ...
        if (event.x() >= gridL && event.x() < gridR
            ...
            if (index >= 0 && index < results.size()) {
                SkyBallsStorageSearch.openResult(Minecraft.getInstance(), results.get(index));
                return true;
            }
        }
