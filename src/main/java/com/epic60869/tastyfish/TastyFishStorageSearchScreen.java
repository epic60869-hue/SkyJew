...
public final class TastyFishStorageSearchScreen extends Screen {
    ...
    private int scroll;
    private List<TastyFishStorageSearch.Result> results = List.of();

    public TastyFishStorageSearchScreen(Screen parent, String query) {
    ...
    private void refresh() {
        ...
        results = TastyFishStorageSearch.search(Minecraft.getInstance(),
    ...
    public void extractRenderState(GuiGraphicsExtractor g, int mouseX, int mouseY, float delta) {
        ...
        int storageCount = TastyFishStorageSearch.cachedStorageCount();
        ...
        for (int row = 0; row < rows; row++) {
            ...
            TastyFishStorageSearch.Result result = results.get(index);
        ...
        long age = TastyFishStorageSearch.oldestCacheAgeMs();
            ...
            if (event.x() >= left + 15 && event.x() <= left + panelWidth() - 15
                ...
                if (row >= 0 && row < rows) {
                    ...
                    if (index >= 0 && index < results.size()) {
                        TastyFishStorageSearch.openResult(Minecraft.getInstance(), results.get(index));
                        return true;
                    }
                }
            }
