...
public final class TastyFishCustomScreen extends Screen {
    ...
    private EquipmentSlot firstEditableArmorSlot() {
        ...
        for (EquipmentSlot slot : slots) {
            ...
            if (!stack.isEmpty() && TastyFishCustom.hasUuid(stack)) return slot;
        }
    ...
    private void addItemFields(int x, int y, int width) {
        ...
        addRenderableWidget(Button.builder(Component.literal("Clear"), b -> {
            TastyFishCustom.setName(target, null);
            init();
    ...
    private void addArmorFields(int x, int y, int width) {
        ...
        addRenderableWidget(Button.builder(Component.literal("Clear"), b -> {
            TastyFishCustom.setTrim(target, null, null);
            init();
        ...
        addRenderableWidget(Button.builder(Component.literal("Clear"), b -> {
            TastyFishCustom.setAnimatedDye(target, null, null, 1, false, 0);
            init();
    ...
    private String currentName() {
        String v = TastyFishCustom.getName(target);
        return v == null ? "" : v;
    }

    private String currentDye() {
        Integer v = TastyFishCustom.getDye(target);
        return v == null ? "" : String.format(Locale.ROOT, "%06X", v);
    }

    private String currentTrimMaterial() {
        TastyFishCustom.TrimId t = TastyFishCustom.getTrim(target);
    ...
    private String currentAnimated(int which) {
        TastyFishCustom.AnimatedDye d = TastyFishCustom.getAnimatedDye(target);
        if (d == null) return "";
    ...
    private void applyName() {
        TastyFishCustom.setName(target, name.getValue());
        init();
    }

    private void applyDye() {
        try {
            TastyFishCustom.setDye(target, TastyFishCustom.parseHex(dye.getValue()));
            init();
        } catch (Exception ignored) {}
    }

    private void applyTrim() {
        TastyFishCustom.setTrim(target, trimMaterial.getValue(), trimPattern.getValue());
        init();
    }
    ...
    private void applyAnimated() {
        try {
            ...
            TastyFishCustom.setAnimatedDye(target, a, b, d, cycleBack, wait);
            init();
        } catch (Exception ignored) {}
    }
    ...
    public void extractRenderState(GuiGraphicsExtractor g, int mouseX, int mouseY, float delta) {
        ...
        if (!target.isEmpty()) {
            ...
            String uuid = TastyFishCustom.uuid(target);
        ...
        if (!TastyFishCustom.hasUuid(target)) {
