...
/** Read-only client for the public Tasty Fish leaderboard API. */
public final class TastyFishWebsiteClient {
    private static final String WEBSITE = "https://tastyfish.org";
    ...
    public record Result(
        ...
        public static Result empty() { return new Result(false, "", "", 0L, -1, "", 0L, -1); }
        public long behind() { return Math.max(0L, aheadValue - value); }
        public boolean hasAheadPlayer() { return available && aheadPosition > 0 && !aheadName.isBlank() && behind() > 0; }
    }
    ...
    public void tick(TastyFishConfig config, String username, UUID uuid, SkysoftSessionReader.Snapshot snapshot) {
        ...
        refresh(crop, username, uuidText).whenComplete((value, error) -> {
    ...
    public void clear() { result = Result.empty(); nextRefreshMillis = 0L; }

    private CompletableFuture<Result> refresh(String crop, String username, String uuid) {
        HttpRequest request = HttpRequest.newBuilder()
            .uri(URI.create(WEBSITE + "/api/leaderboards"))
        ...
        return client.sendAsync(request, HttpResponse.BodyHandlers.ofString()).thenCompose(response -> {
            ...
            if (boardId == null || boardId.isBlank()) return CompletableFuture.completedFuture(Result.empty());
            return fetchBoard(boardId, crop, username, uuid);
        });
    }

    private CompletableFuture<Result> fetchBoard(String boardId, String crop, String username, String uuid) {
        HttpRequest request = HttpRequest.newBuilder()
            .uri(URI.create(WEBSITE + "/api/leaderboards/collection/" + encode(boardId)))
            .timeout(Duration.ofSeconds(10))
            .header("Accept", "application/json")
            .GET().build();
    ...
    private static Result parseBoard(JsonObject board, String boardId, String crop, String username, String uuid) {
        ...
        Row mine = null;
        for (Row row : rows) {
            if (!uuid.isBlank() && uuid.equalsIgnoreCase(row.uuid())) { mine = row; break; }
            if (username.equalsIgnoreCase(row.username())) mine = row;
        }
        if (mine == null) return Result.empty();
        Row ahead = null;
        for (Row row : rows) if (row.position() < mine.position() && (ahead == null || row.position() > ahead.position())) ahead = row;
        return new Result(true, boardId, boardName.isBlank() ? prettyCrop(crop) : boardName, mine.value(), mine.position(),
            ahead == null ? "" : ahead.username(), ahead == null ? 0L : ahead.value(), ahead == null ? -1 : ahead.position());
    }
    ...
    private static String findBoardId(JsonObject root, String crop) {
        ...
        if (root.has("leaderboards") && root.get("leaderboards").isJsonArray()) {
            for (JsonElement element : root.getAsJsonArray("leaderboards")) if (element.isJsonObject()) boards.add(element.getAsJsonObject());
        }
        if (root.has("categories") && root.get("categories").isJsonObject()) {
            for (var entry : root.getAsJsonObject("categories").entrySet()) {
                JsonElement category = entry.getValue();
                if (!category.isJsonArray()) continue;
                for (JsonElement element : category.getAsJsonArray()) if (element.isJsonObject()) boards.add(element.getAsJsonObject());
            }
        }
    ...
    private static List<String> aliases(String crop) {
        ...
        values.add(c);
        switch (c) {
            case "cocoa bean", "cocoa beans", "cocoabean", "cocoabeans" -> { values.add("cocoa"); values.add("cocoabean"); values.add("cocoabeans"); }
    ...
    private static String currentCrop(SkysoftSessionReader.Snapshot snapshot) {
        ...
        for (var entry : snapshot.items().entrySet()) {
            ...
            if (value > count) { count = value; best = prettyCrop(entry.getKey()); }
        }
        return best;
    }
    ...
    private static String prettyCrop(String value) {
        ...
        return value.toLowerCase(Locale.ROOT).replace("minecraft:", "").replace('_', ' ').replace(" item", "").trim();
    }
    ...
    private static String normalizeUuid(String value) { return value == null ? "" : value.replace("-", "").trim().toLowerCase(Locale.ROOT); }
    ...
    private static long number(JsonObject object, String key, long fallback) {
        try { JsonElement value = object.get(key); return value == null || value.isJsonNull() ? fallback : Math.round(value.getAsDouble()); }
        catch (Exception ignored) { return fallback; }
    }
