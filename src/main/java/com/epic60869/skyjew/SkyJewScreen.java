...
public final class SkyJewScreen extends Screen {
    ...
    public void extractRenderState(GuiGraphicsExtractor g, int mouseX, int mouseY, float delta) {
        ...
        g.text(font, "SkyJew", left + 50, top + 14, YELLOW, true);
        ...
        drawToggle(g, x, right, y + 20, "SkyJew enabled", config.enabled, mouseX, mouseY, CYAN);
    ...
    public boolean mouseClicked(MouseButtonEvent event, boolean doubleClick) {
        ...
        if (inside(event.x(), event.y(), x, right, y + 20)) {
            ...
            config.farmingRngY = Math.max(0, config.farmingRngY + (Minecraft.getInstance().hasShiftDown() ? -8 : 8));
        } else {
    ...
    private void save() {
        Path path = Minecraft.getInstance().gameDirectory.toPath().resolve("config").resolve("skyjew-mod.json");
        config.save(path);
    }
