...
import com.epic60869.skyjew.mixin.SkyJewClientTextTooltipAccessor;
 ...
 * The pan resets when you hover something else. SkyJewTooltipMixin wraps the tooltip positioner with
 * {@link #decorate}; SkyJewTooltipScrollMixin hands mouse wheel input to {@link #didHandleMouseScroll}.
...
public final class ScrollableTooltips {
    ...
    private static SkyJewConfig.TooltipScroll config() {
        SkyJewConfig config = SkyJewConfig.current();
        return config == null ? null : config.misc.tooltipScroll;
    }

    private static boolean enabled(SkyJewConfig.TooltipScroll settings) {
    ...
    public static boolean didHandleMouseScroll(double horizontal, double vertical) {
        SkyJewConfig.TooltipScroll settings = config();
    ...
    private static boolean isHorizontalModifierDown(SkyJewConfig.TooltipScroll settings) {
    ...
    private static int identity(List<ClientTooltipComponent> components) {
        Object slot = Minecraft.getInstance().gui.screen() instanceof SkyJewContainerScreenAccessor container ? container.skyjew$getHoveredSlot() : null;
        ClientTooltipComponent title = components.getFirst();
        int result = 31 * Objects.hashCode(slot) + title.getClass().hashCode();
        if (title instanceof SkyJewClientTextTooltipAccessor text) result = 31 * result + textIdentity(text.skyjew$getText());
        return result;
    }
