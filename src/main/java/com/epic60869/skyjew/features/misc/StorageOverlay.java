...
public final class StorageOverlay {
    ...
    private record Header(int x, int y, int w, SkyJewStorageSearch.StoragePage page) {}
    ...
    private static boolean applies(AbstractContainerScreen<?> screen) {
        ...
        return title.equals("Storage") || SkyJewStorageSearch.pageKey(title) != null;
    }
    ...
    private static Layout layout(AbstractContainerScreen<?> screen) {
        ...
        String title = ChatFormatting.stripFormatting(screen.getTitle().getString()).trim();
        String openKey = SkyJewStorageSearch.pageKey(title);
        ...
        List<SkyJewStorageSearch.StoragePage> pages = SkyJewStorageSearch.storagePages();
        for (SkyJewStorageSearch.StoragePage page : pages) {
    ...
    private static void render(AbstractContainerScreen<?> screen, GuiGraphicsExtractor g, int mouseX, int mouseY) {
        ...
        String openKey = SkyJewStorageSearch.pageKey(ChatFormatting.stripFormatting(screen.getTitle().getString()).trim());
        g.nextStratum();
    ...
    private static boolean click(AbstractContainerScreen<?> screen, double mouseX, double mouseY, int button) {
        ...
        for (Header header : layout.headers()) {
            if (mouseX >= header.x() && mouseX < header.x() + header.w() && mouseY >= header.y() && mouseY < header.y() + HEADER
                ...
                SkyJewStorageSearch.StoragePage page = header.page();
