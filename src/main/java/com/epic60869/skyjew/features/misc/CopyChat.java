...
import com.epic60869.skyjew.SkyJewStaff;
import com.epic60869.skyjew.mixin.SkyJewChatComponentAccessor;
...
public final class CopyChat {
    ...
    private static SkyJewConfig.CopyChat config() {
    ...
    public static void init() {
        ScreenEvents.AFTER_INIT.register((client, screen, w, h) -> {
            ...
            ScreenMouseEvents.allowMouseClick(screen).register((s, event) -> {
                SkyJewConfig.CopyChat c = config();
    ...
    private static String stripRanks(String text) {
        String out = text;
        for (SkyJewStaff.Rank rank : SkyJewStaff.allRanks()) out = out.replace("[" + rank.label() + "] ", "");
        return out;
    }
    ...
    private static String hovered(double mouseX, double mouseY, boolean singleLine) {
        ...
        SkyJewChatComponentAccessor access = (SkyJewChatComponentAccessor) chat;
        List<GuiMessage.Line> all = access.skyjew$trimmedMessages();
        ...
        int start = access.skyjew$chatScrollbarPos();
