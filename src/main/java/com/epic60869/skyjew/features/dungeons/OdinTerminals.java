...
public final class OdinTerminals {
    ...
    public static void init() {
        ...
        ClientTickEvents.END_CLIENT_TICK.register(mc -> {
            ...
            if (current == null && enabled() && SkyJewLocation.inDungeon()) open(container);
    ...
    private static void open(AbstractContainerScreen<?> screen) {
        String title = SkyJewLocation.strip(screen.getTitle().getString()).trim();
    ...
    static final class StartsWith extends Handler {
        ...
        List<Integer> solve(List<Slot> slots, int updated) {
            ...
            for (int i = 0; i < slots.size(); i++) {
                ...
                String name = SkyJewLocation.strip(stack.getHoverName().getString()).toLowerCase(Locale.ROOT);
