...
/** /sj debug: prints what SkyJew detects, to diagnose features that do not show. */
public final class SkyJewDebug {
    private SkyJewDebug() {}
    ...
    public static int run() {
        ...
        line("SkyBlock scoreboard", String.valueOf(SkyJewLocation.onSkyblock()));
        line("Area", SkyJewLocation.area());
        line("Location", SkyJewLocation.location());
        line("Dungeon floor", SkyJewLocation.dungeonFloor());
        if (config != null) {
        ...
        if (!held.isEmpty()) {
            ...
            line("Held rarity", SkyJewItemBackgrounds.rarity(held).name());
