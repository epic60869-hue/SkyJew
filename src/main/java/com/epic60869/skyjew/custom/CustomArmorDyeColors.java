				...
				if (!itemUuid.isEmpty()) {
					...
					if (color == Integer.MIN_VALUE) {
						if (customDyeColors.containsKey(itemUuid)) {
							...
							source.sendFeedback(Compat.PREFIX.get().append(Component.translatable("skyjew.customDyeColors.removed")));
						} else {
							source.sendFeedback(Compat.PREFIX.get().append(Component.translatable("skyjew.customDyeColors.neverHad")));
						}
					} else {
						CustomConfigManager.update(config -> config.general.customDyeColors.put(itemUuid, color));
						source.sendFeedback(Compat.PREFIX.get().append(Component.translatable("skyjew.customDyeColors.added")));
					}
				} else {
					source.sendError(Compat.PREFIX.get().append(Component.translatable("skyjew.customDyeColors.noItemUuid")));
				}
			} else {
				source.sendError(Compat.PREFIX.get().append(Component.translatable("skyjew.customDyeColors.notDyeable")));
				return Command.SINGLE_SUCCESS;
			}
		} else {
			source.sendError(Compat.PREFIX.get().append(Component.translatable("skyjew.customDyeColors.unableToSetColor")));
		}
