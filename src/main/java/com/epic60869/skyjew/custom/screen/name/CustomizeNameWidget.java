...
package com.epic60869.skyjew.custom.screen.name;
...
import com.epic60869.skyjew.custom.screen.name.visitor.GetClickedPositionVisitor;
import com.epic60869.skyjew.custom.screen.name.visitor.GetRenderWidthVisitor;
import com.epic60869.skyjew.custom.screen.name.visitor.GetStyleVisitor;
import com.epic60869.skyjew.custom.screen.name.visitor.InsertTextVisitor;
import com.epic60869.skyjew.custom.screen.name.visitor.SetStyleVisitor;
import com.epic60869.skyjew.custom.util.OkLabColor;
import com.epic60869.skyjew.custom.util.SkyBlockColors;
import com.epic60869.skyjew.custom.util.GuiHelper;
...
public class CustomizeNameWidget extends AbstractContainerWidget {
	...
	public CustomizeNameWidget(Screen parent) {
		...
		builder.add(grid.addChild(Button.builder(Component.translatable("skyjew.customItemNames.screen.customColor"), _ ->
				client.gui.setScreen(ColorPopup.create(parent, color -> setStyle(Style.EMPTY.withColor(color))))
		).size(48, 16).build(), 2, 17, 1, 3));
		builder.add(grid.addChild(Button.builder(Component.translatable("skyjew.customItemNames.screen.gradientColor"), _ ->
				client.gui.setScreen(ColorPopup.createGradient(parent, this::createGradient))
		).size(48, 16).build(), 3, 17, 1, 3));
		builder.add(grid.addChild(new StringWidget(20 * 16, textRenderer.lineHeight, Component.translatable("skyjew.customItemNames.screen.howToRemove").withStyle(ChatFormatting.ITALIC, ChatFormatting.GRAY), textRenderer)/*.alignLeft()*/, 4, 0, 1, 20, LayoutSettings.defaults().paddingTop(2)));
	...
	private class FormattingButton extends AbstractButton {
		...
		protected FormattingButton(Component message, ChatFormatting format, Predicate<Style> isEnabled) {
			...
			setTooltip(Tooltip.create(Compat.FORMATTING_FORMATTER.apply(format))); // Yoink from config utils hehhehehehehe
		}
	...
	private class ColorButton extends AbstractButton {
		...
		private ColorButton(ChatFormatting format) {
			super(0, 0, 16, 16, Compat.FORMATTING_FORMATTER.apply(format));
