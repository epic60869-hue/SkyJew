...
public class TrimSelectionWidget extends AbstractContainerWidget {
	...
	public TrimSelectionWidget(int x, int y, int width, int height) {
		...
		Compat.getRegistryWrapperLookup().lookupOrThrow(Registries.TRIM_PATTERN).listElements()
		...
		// Materials
		Compat.getRegistryWrapperLookup().lookupOrThrow(Registries.TRIM_MATERIAL).listElements()
				// Sort them in alphabetical order
				.sorted(Comparator.comparing(reference -> reference.value().description().getString()))
	...
	public void setCurrentItem(ItemStack currentItem) {
		...
		Map<String, CustomArmorTrims.ArmorTrimId> trims = CustomConfigManager.get().general.customArmorTrims;
		String itemUuid = Compat.uuid(currentItem);
