...
import com.epic60869.skyjew.mixin.SkyJewCheckboxAccessor;
import com.epic60869.skyjew.mixin.SkyJewEntityRenderDispatcherAccessor;
...
public class ColorSelectionWidget extends AbstractContainerWidget implements Closeable {
	...
	private static final Component RESET_COLOR_TEXT = Component.translatable("skyjew.customization.armor.resetColor");
	private static final Component PICK_DYE_COLOR_TEXT = Component.translatable("skyjew.customization.armor.pickDye");
	private static final Component CANNOT_CUSTOMIZE_COLOR_TEXT = Component.translatable("skyjew.customization.armor.cannotCustomizeColor");
	private static final Component ANIMATED_TEXT = Component.translatable("skyjew.customization.armor.animated");
	private static final Component CYCLE_BACK_TEXT = Component.translatable("skyjew.customization.armor.cycleBack");
	private static final Component DURATION_TOOLTIP_TEXT = Component.translatable("skyjew.customization.armor.durationTooltip");
	private static final Component DELAY_TOOLTIP_TEXT = Component.translatable("skyjew.customization.armor.delayTooltip");
	private static final String DURATION_TEXT = "skyjew.customization.armor.duration";
	private static final String DELAY_TEXT = "skyjew.customization.armor.delay";
	...
	private void onRemoveCustomColor(Button button) {
		animated = false;
		((SkyJewCheckboxAccessor) animatedCheckbox).setSelected(false);
		changeVisibilities();
	...
	private void onAnimatedCheckbox(Checkbox checkbox, boolean checked) {
		...
		if (animated) {
			...
			((SkyJewCheckboxAccessor) cycleBackCheckbox).setSelected(true);
		} else {
	...
	private void setFromAnimatedDye(Button button, List<CustomArmorAnimatedDyes.Keyframe> keyFrames, boolean cycleBack) {
		this.onRemoveCustomColor(button);
		this.animated = true;
		((SkyJewCheckboxAccessor) animatedCheckbox).setSelected(true);
		((SkyJewCheckboxAccessor) cycleBackCheckbox).setSelected(cycleBack);
	...
	public void refresh() {
		...
		if (key == null) customizable = false;
			...
			EquipmentClientInfo model = ((SkyJewEntityRenderDispatcherAccessor) Minecraft.getInstance().getEntityRenderDispatcher()).getEquipmentAssets().get(key);
		...
		if (CustomConfigManager.get().general.customAnimatedDyes.containsKey(itemUuid)) {
			...
			((SkyJewCheckboxAccessor) cycleBackCheckbox).setSelected(animatedDye.cycleBack());
		...
		changeVisibilities();
		((SkyJewCheckboxAccessor) animatedCheckbox).setSelected(animated);
	}
