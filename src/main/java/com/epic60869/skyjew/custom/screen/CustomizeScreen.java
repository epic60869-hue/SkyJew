...
import com.epic60869.skyjew.custom.CustomConfigManager.GeneralConfig;
...
public class CustomizeScreen extends Screen {
	...
	public CustomizeScreen(@Nullable Screen previousScreen, boolean item) {
		super((Math.random() < 0.01 ? Component.translatable("skyjew.customization.titleSecret") : Component.translatable("skyjew.customization.title")).withStyle(ChatFormatting.GRAY).withStyle(style -> style.withShadowColor(0)));
	...
	private static class CustomizeButton extends AbstractWidget {
		...
		private void updatePosition() {
			SkyJewContainerScreenAccessor accessor = (SkyJewContainerScreenAccessor) inventoryScreen;
			setPosition(accessor.skyjew$getLeftPos() + 63, accessor.skyjew$getTopPos() + 10);
		}
