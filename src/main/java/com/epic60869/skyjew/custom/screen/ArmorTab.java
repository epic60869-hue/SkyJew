...
import static com.epic60869.skyjew.custom.screen.CustomizeScreen.CLIENT;
...
public class ArmorTab extends GridLayoutTab implements Closeable {
	...
	public ArmorTab(CustomizeScreen parent) {
		super(Component.translatable("skyjew.customization.armor"));
		this.parent = parent;
		...
		if (nothingCustomizable) {
			layout.addChild(new StringWidget(Component.translatable("skyjew.customization.nothingCustomizable"), textRenderer), 0, 1, 2, 1, p -> p.alignVerticallyMiddle().alignHorizontallyCenter());
		}
	...
	private class ModelFieldContainer extends AbstractContainerWidget {
		private final Component text = Component.translatable("skyjew.customization.armor.modelOverride").withStyle(ChatFormatting.ITALIC).withStyle(ChatFormatting.GRAY);
		...
		private ModelFieldContainer(int width, int height) {
			...
			field.setTooltip(Tooltip.create(Component.translatable("skyjew.customization.armor.modelOverride.tooltip")));
