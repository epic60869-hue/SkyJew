...
public class ItemSelectPopup extends AbstractPopupScreen {
	...
	private static final Identifier BACKGROUND_TEXTURE = Compat.id("textures/gui/inventory_item_selection.png");
	...
	private class ItemWidget extends AbstractWidget {
		...
		private ItemWidget(ItemStack item) {
			...
			selectable = !Compat.uuid(item).isEmpty();
			Component message = selectable ? getMessage() : getMessage().copy().append("\n").append(Component.translatable("skyjew.customization.item.cannotCustomize").withStyle(ChatFormatting.RED));
