...
import com.epic60869.skyjew.custom.util.AbstractSelectionPopup;
...
public class ModelSelectionPopup extends AbstractSelectionPopup<ModelSelectionPopup.Widget> {
	...
	private final List<Widget> widgets = ((SkyJewModelManagerAccessor) Minecraft.getInstance().getModelManager()).skyjew$getBakedItemStackModels().keySet().stream().sorted().map(Widget::new).toList();
