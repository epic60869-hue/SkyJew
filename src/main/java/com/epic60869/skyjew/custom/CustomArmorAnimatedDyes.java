...
import com.epic60869.skyjew.custom.util.ColorArgumentType;
				...
				if (!itemUuid.isEmpty()) {
					...
					if (color1 == Integer.MIN_VALUE && color2 == Integer.MIN_VALUE) {
						if (customAnimatedDyes.containsKey(itemUuid)) {
							...
							source.sendFeedback(Compat.PREFIX.get().append(Component.translatable("skyjew.customAnimatedDyes.removed")));
						} else {
							source.sendError(Compat.PREFIX.get().append(Component.translatable("skyjew.customAnimatedDyes.neverHad")));
						}
					} else {
						...
						source.sendFeedback(Compat.PREFIX.get().append(Component.translatable("skyjew.customAnimatedDyes.added")));
					}
				} else {
					source.sendError(Compat.PREFIX.get().append(Component.translatable("skyjew.customAnimatedDyes.noItemUuid")));
				}
			} else {
				source.sendError(Compat.PREFIX.get().append(Component.translatable("skyjew.customAnimatedDyes.notDyeable")));
			}
		} else {
			source.sendError(Compat.PREFIX.get().append(Component.translatable("skyjew.customAnimatedDyes.unableToSetDye")));
		}
